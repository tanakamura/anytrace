...
int
ATR_file_open(struct ATR_file *fp, struct ATR *atr, const char *path)
{
    int fd = open(path, O_RDONLY);

    if (fd < -1) {
        ATR_set_libc_path_error(atr, &atr->last_error, errno, path);
        return -1;
    }
    ...
    if (! S_ISREG(st.st_mode)) {
        close(fd);
        ATR_set_unknown_mapped_file_type(atr, &atr->last_error, path);
        return -1;
    }
